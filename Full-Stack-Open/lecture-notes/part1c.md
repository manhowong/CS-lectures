...
- Helper functions can be defined inside a component or outside of it. For example:
    ...
    const Hello = (props) => {
        ...
        // Note: You don't need to pass props to the helper function as it has access to the props of the component
...
- We will define the `Display` and `Button` components outside the `App` component (so they can be reused in other components or projects):
    ...
    ```
    - Note that the prop `onClick` in the `Button` component is different from the `onClick` attribute in the HTML `button` element. You can name the prop anything you want and it doesn't have to match the attribute name in the HTML element.
